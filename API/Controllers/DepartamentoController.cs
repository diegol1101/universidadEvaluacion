...
namespace API.Controllers
{
    public class DepartamentoController : ApiBaseController
    {
        ...
        [HttpGet("ProfesorDepAsig")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<IEnumerable<object>>> ProfesorDepAsig()
        {
            var data = await unitofwork.Departamentos.ProfesorDepAsig();
            return mapper.Map<List<object>>(data);
        }

        [HttpGet("DeptoSinProfesor")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<IEnumerable<object>>> DeptoSinProfesor()
        {
            var data = await unitofwork.Departamentos.DeptoSinProfesor();
            return mapper.Map<List<object>>(data);
        }
    }
}